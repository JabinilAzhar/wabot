...
const configuration = new Configuration({
    apiKey: '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',
});
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (type === "notify" && !messages[0].key.fromMe) {
            try {
                ...
                const whitelist = fs.readFileSync('contacts.txt').toString().split('\n');
                const senderNumberWithoutCountryCode = senderNumber.split('@')[0];
                if (!whitelist.includes(senderNumberWithoutCountryCode)) {

                    console.log('Nomor Pengirim Belum Terdaftar !');
                    await sock.sendMessage(
                        senderNumber,   
                        { text: 'Anda Belum Terdaftar, Silahkan Hubungi ADMIN !!!' },
                        { quoted: messages[0] }
                    );
                    return;
                }
